...
from sphere_math import immersion, chart
import models
...
X0, V, pos = torch.load('Sphere\data\X0.pt').to(device), torch.load('Sphere\data\V.pt').to(device), torch.load('Sphere\data\pos.pt').to(device)
...
for dim, model_list in tqdm(zip(hidden_dims, RNN_models)):
    ...
    for run, model in enumerate(model_list):
        ...
        times.append(t)
        torch.save(model.state_dict(), f'Sphere\model_weights\RNN\hidden_dim_{dim}_{run}.pth')
        pos_pred = torch.zeros_like(pos_test)
        for i in range(n):
            pos_pred[:,i] = model(x_0=X0_test, V=V_test[:,:i+1]).squeeze()
        torch.save(pos_pred, f'Sphere\results\RNN\hidden_dim_{dim}_{run}.pt')
    pickle_list(times, f'Sphere\runtimes_dim_{dim}.pkl')    
