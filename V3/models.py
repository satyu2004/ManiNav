...
class LSTM_multilayer(nn.Module):
    def __init__(self, hidden_size, input_size_x=2, input_size_v=2, output_size=2, num_layers=2):
        super(LSTM_multilayer, self).__init__()
        self.num_layers = num_layers
        self.encoders = [nn.Linear(input_size_x, hidden_size)]*num_layers
        self.lstm = nn.LSTM(input_size_v, hidden_size, batch_first=True)
        self.fc = nn.Linear(hidden_size, output_size)

    def forward(self, x_0, V):
        # Encode x into initial hidden and cell states
        # initial_hidden = torch.tanh(torch.stack([enc(x_0) for enc in self.encoders], dim=0))
        initial_hidden = torch.tanh(self.encoders[0](x_0).unsqueeze(0))  # Using tanh activation
        # initial_hidden = encoded_x.unsqueeze(0)  # Shape (1, batch_size, hidden_size)
        initial_cell = torch.zeros_like(initial_hidden)  # Initialize cell state to zeros
