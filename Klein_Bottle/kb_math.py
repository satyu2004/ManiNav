...
class Klein_Bottle:
    ...
    def immersion(self, point):
        """Immersion. Requires c>a. """

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        u = point[..., 0]
        v = point[..., 1]
        X = (self.c + self.a * torch.cos(v)) * torch.cos(u)
        Y = (self.c + self.a * torch.cos(v)) * torch.sin(u)
        Z = (self.c * torch.sin(v)) * torch.cos(u/2)
        W = (self.c * torch.sin(v)) * torch.sin(u/2)
        return torch.stack(
            [X, Y, Z, W],
            axis=-1,
        ).to(device)
    ...
    def jacobian_matrix_batch(self, pts):
        ...
        jac = torch.stack([torch.autograd.grad(immersed_pts[:, i], pts, torch.ones_like(immersed_pts[:, i]), create_graph=True)[0] for i in range(4)], dim=1)
        return jac
